...
interface SingleExtremeProps {}

const SingleExtreme: React.FC<SingleExtremeProps> = () => {
  const { singleID } = useParams<{ singleID?: string }>();
  const [extremeData, setextremeData] = useState<IEntity.SingleExtremeResponse | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await http.get<IEntity.SingleExtremeResponse>(`/extreme/public/${singleID}`);

        if (response.data && response.data.data) {
          setextremeData(response.data);
        } else {
  ...
  const imgList = extremeData.data.attachList;
  ...
  const priceSection = (
    <div className="mb-4 h-[400px] rounded-lg bg-white p-6 shadow-md">
      ...
      <p className="mb-2 text-sm text-gray-600">Weekday Price: {extremeData.data.weekDayPrice}</p>
      <p className="text-sm text-gray-600">Weekend Price: {extremeData.data.weekendPrice}</p>
  ...
  const ContactSection = (
    <div className="mb-4 rounded-lg bg-white p-6 shadow-md">
      ...
      <p className="mb-2 text-sm text-gray-600">
        Name: {extremeData?.data.profile.firstName} {extremeData?.data.profile.lastName}
      </p>
      <p className="mb-2 text-sm text-gray-600">Phone: {extremeData?.data.profile.phone}</p>
      <p className="mb-2 text-sm text-gray-600">Adress: {extremeData?.data.region.nameUz}</p>
      <p className="mb-2 text-sm text-gray-600">Extreme type: {extremeData?.data.extremeType.name}</p>
    </div>
  );
  ...
  const descriptionSection = (
    <div className="mb-4 rounded-lg bg-white p-6 shadow-md">
      ...
      <p className="text-sm text-gray-600">{extremeData.data.description}</p>
    </div>
  );
