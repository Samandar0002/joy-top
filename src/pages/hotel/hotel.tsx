...
const Hotel: React.FC = () => {
  const [data, setData] = useState<IEntity.HotelRes[]>();
  ...
  const [payload, setPayload] = useState<IApi.Hotel.Request>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const { data } = await Api.HotelList(size, payload);
  ...
  return (
    <>
      <div className="container mx-auto">
        ...
        <h1 className="font-large ml-15 mx-auto my-10 w-[1240px] text-[26px] font-[500]">{t('Mehmonxonalar')}</h1>
        {
                  ...
                  <Link to={`/hotels/joy/${item.id}`}>
                    <div className="card relative m-1">
                      ...
                      <div className="card-info flex flex-col gap-2">
                        ...
                        <p className="card-price text-sm">Srarting price: {item.minPrice}-UZS</p>
